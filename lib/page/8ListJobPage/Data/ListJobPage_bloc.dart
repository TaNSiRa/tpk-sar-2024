...
Stream<int> createListitem() async* {
  ...
  print("itemListJob.length");
  print(itemListJob.length);
  for (var i = 0; i < itemListJob.length; i++) {
    outdata.add({
      "Item_ID": itemListJob[i].itemId,
      "ItemNo": itemListJob[i].itemNo,
      "ItemName": itemListJob[i].itemName,
      "ItemStatus": itemListJob[i].itemStatus,
      "SampleCode": itemListJob[i].sampleCode,
      "JobType": itemListJob[i].jobType,
      "CustFull": itemListJob[i].custFull,
      "SamplingDate": itemListJob[i].samplingDate,
      "SampleGroup": itemListJob[i].sampleGroup,
      "SampleType": itemListJob[i].sampleType,
      "SampleTank": itemListJob[i].sampleTank,
      "SampleName": itemListJob[i].sampleName,
      "Position": itemListJob[i].position,
      "Mag": itemListJob[i].mag,
      "Temp": itemListJob[i].temp,
      "StdFactor": itemListJob[i].stdFactor,
      "StdMax": itemListJob[i].stdMax,
      "StdMin": itemListJob[i].stdMin,
      "ReceiveDate": itemListJob[i].receiveDate,
      "AnalysisDueDate": itemListJob[i].analysisDueDate,
      "SampleRemark": itemListJob[i].sampleRemark,
      "Selected": itemListJob[i].selected,
      "SampleAmount": itemListJob[i].sampleAmount,
      "ErrorName": itemListJob[i].errorName,
      "ErrorStatus": itemListJob[i].errorStatus,
      "CustShort": itemListJob[i].custShort,
    });
  }
  ...
  try {
    ...
    final response = await Dio().post(
      url + '/ListJobPage_CreateListItem',
      data: qParams,
    );
